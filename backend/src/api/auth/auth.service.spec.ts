...
describe('AuthService', () => {
  ...
  describe('login', () => {
    it('should throw an HttpException with 401 if user is not found', async () => {
      ...
      await expect(service.login(loginDto)).rejects.toHaveProperty(
        'status',
        401,
      );
    });
  ...
  describe('register', () => {
    it('should throw an HttpException with 401 if user already exists', async () => {
      ...
      await expect(service.register(registerDto)).rejects.toHaveProperty(
        'status',
        401,
      );
    });
  ...
  describe('verifyUser', () => {
    it('should throw an HttpException with 401 if user is not found', async () => {
      ...
      await expect(service.verifyUser(userId)).rejects.toThrowError(
        'USER_NOT_FOUND',
      );
      await expect(service.verifyUser(userId)).rejects.toHaveProperty(
        'status',
        401,
      );
    });
